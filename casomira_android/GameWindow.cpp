...
GameWindow::GameWindow(std::string nazev, int width, int height, int flag, int x, int y) {
    ...
    if(window == NULL)
    {
       SDL_Log("Okno nemohlo byt vytvoreno! SDL_ERROR: %s\n", SDL_GetError());
        konsChyba = true;
    }
    //vytvoreni rendereru
...
bool GameWindow::createRenderer()
{
    ...
    if(renderer == NULL)
    {
        SDL_Log("Renderer nemohl byt vytvoren! SDL_ERROR: %s\n", SDL_GetError());
        return 0;
    }
...
bool GameWindow::update(Image *obrazek)
{
    ...
    if(textura == NULL)
    {
        SDL_Log("Textura se nemohla vystvorit!", SDL_GetError());
        return 0;
    }
    ...
    if(i!=0)
    {
        SDL_Log("Nelze vyrendrovat", SDL_GetError());
